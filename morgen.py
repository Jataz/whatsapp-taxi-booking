...
@app.route('/dashboard')
def dashboard():
    m = folium.Map(location=[-17.833626443125766, 31.018526778543766],zoom_start=14)
...
#-----------------------AFTER USER SENDS LOCATIONS---------------------
            ...
            for phoneNumberr in Numberr:
                #Account SID 
                account_sid = "AC3fd0e270f2cd2e32f4a9c108387e866a"
                # Account Auth Token 
                auth_token  = "447ba267aebcf4a3356e4e790359b00b"
...
#------------------------------------------------------------------------------------------------------
            while minDistance > 0:
                ...
                connection = sqlite3.connect('WhatsappTaxiOrg.db')
                cursor = connection.cursor()
                for row in cursor.execute("SELECT lat, lon FROM coordinates "):
                    message_latitude1, message_longitude1 = row[0], row[1]
